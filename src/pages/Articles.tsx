...
const Articles = () => {
    ...
    const { data: { articles = [], total_count = 0 } = {}, isLoading, isError, error } = useArticles({ page, topic, sortBy, order });
    ...
    return isError ? <h2>{`${error.name}: ${error.message}`}</h2> : isLoading ? <Loading /> : <section className="articles-page">
        ...
        <PageButtons page={page ?? '1'} totalPages={Math.ceil(total_count / 10) || 1} searchParams={searchParams} setSearchParams={setSearchParams}></PageButtons>
    </section>
}

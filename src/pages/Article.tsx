...
const Article = () => {
    ...
    const { data: articleData, isLoading, isError, error } = useArticle(Number(article_id));
    const [isDeleted, setIsDeleted] = useState(false);

    return isDeleted ? <h2>Article Deleted Succsessfully</h2> : isError ? <h2>{`${error.name}: ${error.message}`}</h2> : isLoading ? <Loading></Loading> : <section className="article-page">
